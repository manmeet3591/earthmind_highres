...
================================
Diffusion-based downscaling of GraphCast forecasts to AORC-resolution
fields over an arbitrary bounding box, using a trained UNet3D LCM model.
Usage example:
...
def infer_batch(
    ...
    log(f"  [denoise] completed {num_steps} steps in {total_denoise:.2f}s")
...
def run(args: argparse.Namespace) -> None:
    ...
    # -------------------------------------------------------- static grids
    log("Loading static grids (topo + svf)…")
    ...
    for steps in tqdm(step_list, desc="diffusion-steps", unit="cfg"):
        ...
        cfg_start = time.time()
        log(f"\n{'─'*72}")
        log("Starting diffusion config: steps={steps}")
        log(f"{'─'*72}")
